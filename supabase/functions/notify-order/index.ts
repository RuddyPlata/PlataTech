...
// Triggered by Database Webhooks on INSERT and UPDATE to the `orders` table.
// INSERT → nueva orden: email al dueño + confirmación al cliente
// UPDATE → cambio de estado: solo email al cliente (si el status cambió)
//
...
// ── Status labels ─────────────────────────────────────────────────────────────
const STATUS_INFO: Record<string, { label: string; color: string; msg: string }> = {
  pagado:     { label: 'Pago confirmado',       color: '#059669', msg: 'Tu pago fue confirmado. Estamos preparando tu pedido.' },
  contactado: { label: 'Te contactamos',        color: C_BLUE,    msg: 'Un representante de Plata Tech ya revisó tu pedido y se está poniendo en contacto contigo por WhatsApp.' },
  encamino:   { label: '¡Tu pedido va en camino!', color: '#d97706', msg: 'Tu pedido salió y está siendo entregado. Mantente pendiente.' },
  entregado:  { label: 'Pedido entregado',      color: '#059669', msg: '¡Tu pedido fue entregado exitosamente! Gracias por confiar en Plata Tech.' },
  cancelado:  { label: 'Pedido cancelado',      color: '#dc2626', msg: 'Tu pedido fue cancelado. Si tienes preguntas, escríbenos por WhatsApp.' },
};
...
serve(async (req: Request) => {
  ...
  if (eventType === 'UPDATE') {
    ...
    if (!customer?.email) {
      return new Response(JSON.stringify({ ok: true, skipped: 'no_email' }), { headers: { 'Content-Type': 'application/json' } });
    }
    ...
    const ok = await sendEmail(
      customer.email,
      `Tu orden ${r.id} — ${info.label}`,
      html
    );
    return new Response(JSON.stringify({ ok, order: r.id, newStatus }), { headers: { 'Content-Type': 'application/json' } });
  }
  ...
  const [ownerOk, customerOk] = await Promise.all([
    sendEmail(
      ownerEmail,
      `Nueva orden ${r.id} — ${customer?.name ?? 'cliente'} · ${fmt(totals?.total ?? 0)}`,
      ownerHtml
    ),
    customer?.email
      ? sendEmail(
